...
namespace MyBinaryHeap
{
    ...
    private:
        ...
        static void sift_down(Type *data, size_t size, size_t index)
        {
            ...
            while (ch < size)
            {
                if (ch < size - 1 && data[ch] > data[ch + 1])
                    ch++;
                if (data[ch] < val)
                {
        ...
        template <typename Type>
        friend void heapSort(Type *data, size_t size);
    ...
    inline void heapSort(Type *data, size_t size)
    {
        ...
        {
            MyBinaryHeap<Type>::sift_down(data, size, pos);
        }

        for (int pos = (int)size - 1; pos > 0; --pos)
        {
            std::swap(data[0], data[pos]);
            MyBinaryHeap<Type>::sift_down(data, pos, 0);
        }
